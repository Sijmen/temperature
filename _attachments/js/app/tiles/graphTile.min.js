GraphTile=Class.create(Tile,{initialize:function(a,b,c){var d=this;this.oOptions=$.extend({timespan:300,smoothScale:1,timespanType:"time",serie:{name:"serie1",data:[],color:"#c05020"}},c);
this.vInit();this.sSelector=a;this.bRendered=false;this.oGraph=null;this.oDataProvider=b;this.aSeries=[];this.oSerie=this.oOptions.serie;this.aSeries.push(this.oSerie);
...
},vRender:function(){if(this.bRendered){this.vUpdate();}this.oGraph=new Rickshaw.Graph({element:document.querySelector(this.sSelector),renderer:"area",width:$(this.sSelector).parent().innerWidth(),height:350,min:"auto",padding:{top:0.1,bottom:0.1},stroke:false,series:this.aSeries});
...
},10);}else{if(this.bRendered){this.oGraph.render();}else{this.vRender();}}},vSetSmooth:function(a){this.oOptions.smoothScale=a;},vResize:function(){console.log("Resizing in graphTile "+this.sSelector);
this.oGraph.configure({width:$(this.sSelector).parent().innerWidth()});this.vUpdate();},vSetTimespan:function(b,a){if(b>a){this.vSetTimespanSeconds(Math.round(b-a));
}else{this.vSetTimespanSeconds(Math.round(a-b));}},vSetTimespanSeconds:function(a){this.iTimespan=a;}});