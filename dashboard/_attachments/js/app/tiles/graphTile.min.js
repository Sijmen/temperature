GraphTile=Class.create(Tile,{initialize:function($super,a,d,b){var c=this;$super(a,b);this.oOptions=$.extend(true,{timespan:300,smoothScale:1,timespanType:"time",serie:{name:"serie1",data:[],color:"#c05020"},graphOptions:{min:"auto",renderer:"area"}},b);
...
});},vAddSerie:function(c,a,b){this.aSeries.push(a);this.oSeries[a.name]=a;this.zipped=this.zipped.combinelatest(c,function(e,d){e.push({serie:a.name,data:d});
return e;});},vRender:function($super){var a=this;if(!a.disposable){a.disposable=a.zipped.subscribe(function(b){Rx.Observable.fromArray(b).subscribe(function(c){a.vAddData(c.data,a.oSeries[c.serie]);
});});}if(this.aSeries[0].data.length>0&&!a.oGraph){a.oGraph=new Rickshaw.Graph($.extend(true,{element:document.querySelector(a.sSelector),renderer:"area",width:$(a.sSelector).parent().innerWidth(),height:350,min:"auto",padding:{top:0.1,bottom:0.1},series:a.aSeries},a.oOptions.graphOptions));
a.hoverDetail=new Rickshaw.Graph.HoverDetail({graph:a.oGraph});a.smoother=new Rickshaw.Graph.Smoother({graph:a.oGraph});a.smoother.setScale(a.oOptions.smoothScale);
a.xAxis=new Rickshaw.Graph.Axis.Time({graph:a.oGraph,timeFixture:new Rickshaw.Fixtures.Time.Local()});a.yAxis=new Rickshaw.Graph.Axis.Y({graph:a.oGraph});
