GraphTile=Class.create(Tile,{initialize:function($super,a,d,b){var c=this;this.independentSeries=[];$super(a,b);this.oOptions=$.extend(true,{timespan:300,smoothScale:1,timespanType:"time",serie:{name:"serie1",data:[],color:"#c05020"},graphOptions:{min:"auto",renderer:"area"}},b);
...
});},vAddSerie:function(c,a,b){this.aSeries.push(a);this.oSeries[a.name]=a;console.log(this.zipped);this.zipped=this.zipped.zip(c,function(e,d){e.push({serie:a.name,data:d});
return e;});},addIndependentSerie:function(c,a,b){console.log(c);console.log(a);console.log(b);this.aSeries.push(a);this.oSeries[a.name]=a;this.independentSeries.push(c.map(function(d){return{data:d,serie:a.name};
...
});});Rx.Observable.fromArray(this.independentSeries).subscribe(function(b){b.subscribe(function(c){a.vAddData(c.data,a.oSeries[c.serie]);});});}if(this.aSeries[0].data.length>0&&!a.oGraph){a.oGraph=new Rickshaw.Graph($.extend(true,{element:document.querySelector(a.sSelector),renderer:"area",width:$(a.sSelector).parent().innerWidth(),height:350,min:"auto",padding:{top:0.1,bottom:0.1},series:a.aSeries},a.oOptions.graphOptions));
